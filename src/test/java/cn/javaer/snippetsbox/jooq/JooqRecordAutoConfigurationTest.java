...
class JooqRecordAutoConfigurationTest {
    ...
    void jooqCodegen() throws Exception {
        JdbcTemplate jdbcTemplate = new JdbcTemplate(new TestConfiguration().dataSource());
        jdbcTemplate.execute("CREATE TABLE city ( id  INTEGER IDENTITY PRIMARY KEY, name VARCHAR(30) )");
    ...
    static class AssertController {
        ...
        @PostMapping("/")
        static String save(@RequestBody CityRecord updatableRecord) {
        ...
        @GetMapping("/{id}")
        static String get(@PathVariable("id") CityRecord updatableRecord) {
